...
`v0.1.0` erzeugt. Der GitHub-Workflow baut eine self-contained x64-Anwendung,
...
4. Die erforderlichen Fälle aus [manual-testing.md](manual-testing.md) mit dem
   Release-Kandidaten durchführen und protokollieren.
5. Installer, Update über eine vorhandene Version und Deinstallation manuell
   prüfen.

Ein Release ist nicht allein aufgrund eines grünen CI-Builds freigabefähig. Die
Hardwaretests für Freeze, Refresh, DPI und Monitoränderungen sind verpflichtend.
...
```powershell
git tag -a v0.1.0 -m "LageFreeze v0.1.0"
git push origin v0.1.0
```

Der Workflow akzeptiert stabile Tags und Vorabversionen wie `v0.2.0-beta.1`.
...
Die Vorabversionen sind mangels bereitgestelltem Zertifikat nicht digital
signiert. Der dadurch mögliche SmartScreen-Hinweis wird in README und Release
Notes transparent genannt. Code Signing wird erst ergänzt, wenn Zertifikat und
sichere Secret-Verwaltung festgelegt sind.
...
& "${env:ProgramFiles(x86)}\Inno Setup 6\ISCC.exe" "/DMyAppVersion=0.1.0" "/DMyAppNumericVersion=0.1.0.0" "/DPublishDir=$PWD\artifacts\publish\win-x64" "/DOutputDir=$PWD\artifacts" "installer\LageFreeze.iss"
```
